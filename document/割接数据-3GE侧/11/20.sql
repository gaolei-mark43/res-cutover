...
select /*+parallel(t,10)*/
 ...
 (select mp.province_name
   ...
   where mp.province_code = mb.province_code and mp.province_code in ('11')) as PROVINCE_NAME,
 (select me.eparchy_name
    from td_m_eparchy me
   where me.eparchy_code = t.apply_eparchy_code and me.province_code in ('11')) as EPARCHY_NAME,
 (select ma.area_name
    from td_m_area ma
 ...
 (select ms.staff_name
    from tf_m_staff ms
   where ms.staff_id = t.apply_staff_id and ms.province_code in ('11')) STAFF_NAME,
 0 SALE_COUNT,
 '' ACTIVITY_TYPE,
 '联通自营' PLATFORM_DESC,
 (select ms.staff_name
    from tf_m_staff ms
   where ms.staff_id = t.apply_staff_id and ms.province_code in ('11')) UPDATE_STAFF_NAME,
...
select /*+parallel(t,10)*/
 ...
 (select ms.staff_name
    from tf_m_staff ms
   where ms.staff_id = t.oper_staff_id and ms.province_code in ('11')) STAFF_NAME,
 0 SALE_COUNT,
 '' ACTIVITY_TYPE,
 '联通自营' PLATFORM_DESC,
 (select ms.staff_name
    from tf_m_staff ms
   where ms.staff_id = t.oper_staff_id and ms.province_code in ('11')) UPDATE_STAFF_NAME,
 (select ti.intproxy_name
...
insert /*+append*/ into rc_order
 ...
 (select ti.intproxy_name
   ...
   where od.send_trade_id = t.send_trade_id
     ...
     and od.province_id in ('11')
     and rownum = 1) FACTORY_NAME,
(select t1.stock_name from tf_r_stock t1 where t1.stock_id=t.send_stock_id and t1.res_type_code='1' and t1.province_id in ('11')) DEPOT_ID_IN_NAME,
 '' DEPOT_ID_OUT_NAME,
'' AUDIT_DEPART,
...
select /*+parallel(t,10)*/
 ...
 and  t.order_bill_state='4'
   and t.province_id in ('11')
   and exists(select 1 from tf_r_tml_order tc where tc.relate_order_id=t.order_log_id and tc.order_bill_state in ('0','1','2','3','4') and tc.province_id in ('11'))
 group by t.order_log_id,
          t.apply_stock_id,
          t.order_bill_state,
          t.apply_depart_id,
...
'' FACTORY_CODE,
 ...
 mb.depart_name DEPART_NAME,
(select ms.staff_name from tf_m_staff ms where ms.staff_id=t.return_staff_id and ms.province_code in ('11')) STAFF_NAME,
 t.res_num SALE_COUNT,
 '' ACTIVITY_TYPE,
 '联通自营' PLATFORM_DESC,
(select ms.staff_name from tf_m_staff ms where ms.staff_id=t.return_staff_id and ms.province_code in ('11')) UPDATE_STAFF_NAME,
'' FACTORY_NAME,
'' DEPOT_ID_IN_NAME,
(select t2.stock_name from tf_r_stock t2 where t2.stock_id=t.stock and t2.res_type_code='1' and t2.province_id in ('11')) DEPOT_ID_OUT_NAME,
