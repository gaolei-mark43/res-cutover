...
SELECT /*+parallel(t,10)*/
 ...
 (select md.bss_depart_code
    from td_m_depart md
   where md.depart_id = rs.depart_id and md.province_code in ('97'))  DEPART_IN,
 ...
 (select me.eparchy_name
   ...
   where me.eparchy_code = t.eparchy_code and me.province_code in ('97')) eparchyName,
 (select md.depart_name
    from td_m_depart md
   where md.depart_id = rs.depart_id and md.province_code in ('97')) departInName,
 (select mf.staff_name from tf_m_staff mf where mf.staff_id=t.staff_id_in and mf.province_code in ('97')) staffInName,
...
insert /*+append*/into RC_STOCK
 SELECT /*+parallel(t,10)*/
   trim(t.terminal_id) RS_IMEI,
   rr.rsid RS_ID,
   (case when exists (select 1 from tf_r_stock rs where rs.depart_id=rt.channel_id and rs.res_type_code='1' and rs.valid_flag='1' and rs.province_id in ('97'))
  then (select rs.stock_id from tf_r_stock rs where rs.depart_id=rt.channel_id and rs.res_type_code='1' and rs.valid_flag='1' and rs.province_id in ('97') and rownum=1)
    else case when exists (select 1 from tf_r_stock rs where rs.depart_id=rt.channel_id and rs.res_type_code='1' and rs.valid_flag='0' and rs.province_id in ('97'))
      and not exists (select 1 from tf_r_stock rs where rs.depart_id=rt.channel_id and rs.res_type_code='1' and rs.valid_flag='1' and rs.province_id in ('97')) 
  then (select rs.stock_id from tf_r_stock rs where rs.depart_id=rt.channel_id and rs.res_type_code='1' and rs.valid_flag='0' and rownum=1 and rs.province_id in ('97'))
   else 
 ...
 (case
  ...
  (select mf.staff_name from tf_m_staff mf where mf.staff_id=rt.record_staff and mf.province_code in ('97')) staffInName,
   a.terminal_type_code resTypeCode,
   (SELECT p.param_name
      FROM TD_B_COMMPARA P
     WHERE P.PARAM_ATTR = '1520'
...
insert /*+append*/into RC_STOCK
  SELECT /*+parallel(ah,10)*/
   ...
   (select cc.city_id
      from tf_c_channel cc
     where cc.channel_code = ah.channel_code and cc.province_id in ('97')) EPARCHY_CODE,
   '123' SHEL_NO,
   '456' SEAT_NO,
   ...
   (select cc.channel_name
      from tf_c_channel cc
     where cc.channel_code = ah.channel_code and cc.province_id in ('97')) depotName,
   ...
   (select me.eparchy_name
     ...
     where cc.channel_code = ah.channel_code
       and cc.city_id = me.eparchy_code and me.province_code in ('97')) eparchyName,
   (select cc.channel_name
      from tf_c_channel cc
     where cc.channel_code = ah.channel_code and cc.province_id in ('97')) departInName,
   '3GESS割接' staffInName,
   a.terminal_type_code resTypeCode,
   (SELECT p.param_name
      FROM TD_B_COMMPARA P
