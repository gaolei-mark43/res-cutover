...
t.province_id || SEQ_res_detail_ID.Nextval OPER_ID,
 ...
 (select me.eparchy_name
   ...
   where me.eparchy_code = t.eparchy_code and me.province_code in ('84')) as EPARCHY_NAME,
 (select ma.area_name
    from td_m_area ma
   where ma.province_code = mb.province_code
     and ma.bss_area_code = mb.city_code
     and ma.province_code in ('84')
     and ma.validflag = '10') as CITY_NAME,
         ...
         (select t1.stock_name from tf_r_stock t1 where t1.stock_id=t.send_stock_id and t1.res_type_code='1' and t1.province_id in ('84')) DEPOT_NAME,
...
ro.oper_id DEPOT_OPER_ID, --_class
         ...
         a.res_num, --orderNum
         a.res_num,
         case when f.bill_state='2' then a.res_num else (select count(*) from tf_r_tml_consig_detail t where t.send_trade_id=a.send_trade_id and t.property_id=a.property_id and t.province_id in ('84')) end,
         '', --updateStaffId
...
a.province_id || SEQ_res_detail_ID.Nextval tosd_id,
        ro.oper_id depot_oper_id,
         ...
         e.res_model_code,
         e.res_model_desc,
         (select tf.staff_name from tf_m_staff tf where tf.staff_id=f.update_staff_id and tf.province_code in ('84')) update_staff_name,
         '1',
         '3GESS割接' || '-' || to_char(sysdate, 'yyyymmdd') || '-' || a.province_id remark
 ...
 decode(t.trade_type,
   ...
   (select t1.stock_name
     ...
     where t1.stock_id = t.stock_id_out
       and t1.res_type_code = '1' and t1.province_id in ('84')) DEPOT_NAME,
   '3GESS割接' || '-' || to_char(sysdate, 'yyyymmdd') || '-' || t.province_id remark
...
t.province_id  || SEQ_res_detail_ID.Nextval OPER_ID,
   ...
   (select mp.province_name
     ...
     and mp.province_code in ('84')) as PROVINCE_NAME,
   (select me.eparchy_name
      from td_m_eparchy me
     where me.eparchy_code = t.eparchy_code and me.province_code in ('84')) as EPARCHY_NAME,
   (select ma.area_name
...
ro.oper_id DEPOT_OPER_ID, --_class
         ...
         a.oper_num, --orderNum
         a.oper_num,
         case when f1.log_state='3' then a.oper_num else (select count(*) from tf_r_tml_transf_detail td where td.log_id=a.log_id and td.property_id=a.property_id and td.province_id in ('84') and td.entry_id is not null) end,
         '', --updateStaffId
         f1.create_date, --updateTime
         e.rs_name,
         e.res_type_code,
...
a.province_id || SEQ_res_detail_ID.Nextval tosd_id,
        ro.oper_id depot_oper_id,
         ...
         e.res_model_code,
         e.res_model_desc,
         (select tf.staff_name from tf_m_staff tf where tf.staff_id=a.out_staff_id and tf.province_code in ('84')) update_staff_name,
         '1',
         '3GESS割接' || '-' || to_char(sysdate, 'yyyymmdd') || '-' || a.province_id remark
...
a.province_id || SEQ_res_detail_ID.Nextval tosd_id,
        ro.oper_id depot_oper_id,
         ...
         e.res_model_code,
         e.res_model_desc,
         (select tf.staff_name from tf_m_staff tf where tf.staff_id=a.in_staff_id and tf.province_code in ('84')) update_staff_name,
         '1',
         '3GESS割接' || '-' || to_char(sysdate, 'yyyymmdd') || '-' || a.province_id remark
  from Tf_r_Tml_Transf_Detail a,
         TD_R_TERMINAL_PROPTY  b,
 ...
 (select ma.area_name
   ...
   where ma.province_code = mb.province_code
     ...
     and ma.validflag = '10') as CITY_NAME,
         ...
         '10' PLATFORM,
          t.stock  DEPOT_ID,
         (select t1.stock_name from tf_r_stock t1 where t1.stock_id=t.stock and t1.res_type_code='1' and t1.province_id in ('84')) DEPOT_NAME,
 '3GESS割接' || '-' || to_char(sysdate, 'yyyymmdd') || '-' || t.province_id remark
...
select /*+parallel(t,10)*/
 ...
 (select me.eparchy_name
   ...
   and me.province_code in ('84')) as EPARCHY_NAME,
 (select ma.area_name
    from td_m_area ma
   where ma.province_code = mb.province_code
     and ma.bss_area_code = mb.city_code
...
ro.oper_id DEPOT_OPER_ID, --_class
         ...
         a.res_num, --orderNum
         a.res_num,
         (select count(*) from tf_r_tml_return_detail rd where rd.log_id=a.log_id and rd.property_id=a.property_id and rd.terminal_state='1' and rd.province_id in ('84')),
         '', --updateStaffId
         f1.update_date, --updateTime
         e.rs_name,
         e.res_type_code,
...
a.province_id || SEQ_res_detail_ID.Nextval tosd_id,
        ro.oper_id depot_oper_id,
         ...
         e.res_model_code,
         e.res_model_desc,
         (select tf.staff_name from tf_m_staff tf where tf.staff_id=f.out_staff_id and tf.province_code in ('84')) update_staff_name,
         '1',
         '3GESS割接' || '-' || to_char(sysdate, 'yyyymmdd') || '-' || a.province_id remark
...
a.province_id || SEQ_res_detail_ID.Nextval tosd_id,
        ro.oper_id depot_oper_id,
         ...
         e.res_model_code,
         e.res_model_desc,
         (select tf.staff_name from tf_m_staff tf where tf.staff_id=f.confirm_staff and tf.province_code in ('84')) update_staff_name,
         '1',
         '3GESS割接' || '-' || to_char(sysdate, 'yyyymmdd') || '-' || a.province_id remark
  from Tf_r_Tml_Return_Detail a,
         TD_R_TERMINAL_PROPTY  b,
