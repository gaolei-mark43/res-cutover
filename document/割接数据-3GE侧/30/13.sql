...
select /*+full(t) parallel(t,10)*/
 ...
 (select me.eparchy_name
   ...
   where me.eparchy_code = t.subscribe_eparchy and me.province_code in ('30')) as EPARCHY_NAME,
 (select ma.area_name
    from td_m_area ma
 ...
 where t.resource_id = ts.terminal_id
   ...
   and t.subscribe_province in ('30')
   and mb.province_code in ('30')
   and t.subscribe_depart = mb.depart_id
   and ms.staff_id = t.subscribe_staff
   and ms.province_code in ('30')
...
t.saletrade_id link_order_id,
 ...
 tc.factory_name FACTORY_NAME,
 '' DEPOT_ID_IN_NAME,
 (select rs.stock_name from tf_r_stock rs where t.sale_stock=rs.stock_id and rs.res_type_code='1' and rs.province_id in ('30')) DEPOT_ID_OUT_NAME,
 '' AUDIT_DEPART,
 sysdate START_TIME,
 sysdate END_TIME,
 '' AUTHORIZE_DEPOT_ID,
